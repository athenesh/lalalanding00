...
-- 파일 경로의 name 파라미터를 사용하여 첫 번째 폴더명을 추출
...
WITH CHECK (
  bucket_id = 'uploads' AND (
    ...
    -- 파일 경로의 첫 번째 폴더명이 클라이언트의 clerk_user_id와 일치하는지 확인
    EXISTS (
...
-- STEP 7: 최종 확인 (반드시 실행!)
-- ============================================
-- 정책의 실제 내용을 확인하여 clients.name이 사용되지 않았는지 확인
SELECT 
  '최종 확인' as step,
  policyname,
  cmd,
