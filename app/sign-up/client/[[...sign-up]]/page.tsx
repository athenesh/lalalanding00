...
export default function ClientSignUpPage() {
  ...
  useEffect(() => {
    ...
    if (userId && user) {
      ...
      // 역할이 없으면 회원가입 완료 페이지로 리다이렉트 (토큰 포함)
      if (!role) {
        const token = searchParams.get("token");
        const redirectUrl = token
          ? `/sign-up/client/complete?token=${token}`
          : "/sign-up/client/complete";
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-80px)] px-4">
          <div className="w-full max-w-md space-y-4">
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {invitationToken
                  ? "유효하지 않은 초대 링크입니다. 에이전트에게 새로운 초대 링크를 요청해주세요."
                  : "이 서비스는 에이전트의 초대를 통해서만 가입할 수 있습니다. 에이전트에게 초대 링크를 요청해주세요."}
              </AlertDescription>
            </Alert>
        ...
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="text-center mb-2">
            <p className="text-sm text-muted-foreground">
              이미 계정이 있으신가요?{" "}
              <Link
                href={`/sign-in?redirect=/sign-up/client/complete?token=${invitationToken}`}
                className="text-primary hover:underline font-medium"
              >
                로그인하기
              </Link>
          ...
          <SignUp
            ...
            fallbackRedirectUrl={`/sign-up/client/complete?token=${invitationToken}`}
            signInFallbackRedirectUrl={`/sign-in?redirect=/sign-up/client/complete?token=${invitationToken}`}
            routing="path"
            path="/sign-up/client"
          />
        </div>
