...
import { updateHousingSchema, uuidSchema } from "@/lib/validations/api-schemas";
...
export async function GET(
  ...
  try {
    ...
    // UUID 검증
    const idValidation = uuidSchema.safeParse(client_id);
...
export async function PATCH(
  ...
  try {
    ...
    const updateData: any = {
      ...
      budget_max: maxBudget, // 이미 스키마에서 숫자로 변환됨
      housing_type: housingTypeArray,
      bedrooms: bedrooms, // 이미 스키마에서 숫자로 변환됨
      bathrooms: bathrooms, // 이미 스키마에서 숫자로 변환됨
      furnished: furnished ?? null,
      has_washer_dryer: hasWasherDryer ?? null,
      parking: parking ?? null,
      parking_count: parkingCountInt, // 이미 스키마에서 숫자로 변환됨
