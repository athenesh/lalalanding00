...
SELECT 
  ...
  COUNT(CASE WHEN with_check LIKE '%storage.foldername(clients.name)%' OR qual LIKE '%storage.foldername(clients.name)%' THEN 1 END) as wrong_policies
FROM pg_policies
WHERE schemaname = 'storage' 
  AND tablename = 'objects';

-- ============================================
-- STEP 1: 모든 Storage 정책 강제 삭제
-- ============================================
