...
**⚠️ 중요: 체크리스트 템플릿은 표준화된 데이터이므로 함부로 수정하지 마세요.**

- **템플릿 구조**: 총 16개 항목으로 고정
...
- **템플릿 수정 규칙**:
  ...
  - `checklist_templates` 테이블의 데이터는 **절대 직접 수정하지 않음**
  - 템플릿 변경이 필요한 경우:
    1. `docs/정리.md` 파일을 먼저 확인하여 표준 구조 확인
    2. 사용자에게 변경 사유와 내용을 명확히 확인받음
    3. 마이그레이션 파일을 통해서만 변경 (`supabase/migrations/`)
    4. 변경 후 `docs/정리.md` 파일도 함께 업데이트

- **템플릿 참조**:

  - 표준 템플릿 구조는 `docs/정리.md` 파일을 참조
  - 카테고리는 한글 기준으로 통일: `출국 전 준비`, `입국 직후`, `정착 초기`, `정착 완료`

- **클라이언트별 체크리스트**:
  - `checklist_items` 테이블은 클라이언트별로 템플릿에서 복사되어 생성됨
  - 클라이언트별 데이터는 자유롭게 수정 가능 (완료 상태, 메모, 파일 등)
...
- `checklist_templates`: 체크리스트 표준 템플릿 (16개 고정)

  - **절대 직접 수정 금지**: 마이그레이션을 통해서만 변경
  - 표준 구조는 `docs/정리.md` 참조

- `checklist_items`: 클라이언트별 체크리스트 (템플릿에서 복사)
  - 클라이언트별 데이터는 자유롭게 수정 가능
...
NEXT_PUBLIC_CLERK_SIGN_IN_FALLBACK_REDIRECT_URL=/client/home
